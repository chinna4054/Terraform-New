variable "region" {
  type        = string
  description = "Your region"
  default = "us-east-1"
}

variable "broker_name" {
  type        = string
  description = "Name of the broker"
  default = "chaikin-dev"
}

variable "security_groups" {
  type        = list(string)
  description = "List of security group IDs assigned to the broker"
  default = [ "sg-064bc77757dc389e1" ]
}

variable "subnet_id" {
  type        = list(string)
  description = "subnet of your choice"
  default = ["subnet-0086aeadb2777a370" ]
}

variable "host_instance_type" {
  type        = string
  description = "Broker's instance type"
  default = "mq.t3.micro"
}

variable "engine_type" {
  type        = string
  description = "Type of broker engine"
  default     = "RabbitMQ"
}

variable "engine_version" {
  type        = string
  description = "Version of the broker engine"
  default     = "3.11.20"
}

variable "storage_type" {
  type        = string
  description = "Storage type of the broker,"
  default     = null
}

variable "authentication_strategy" {
  type        = string
  description = "Authentication strategy used to secure the broker"
  default     = "simple"
}

variable "deployment_mode" {
  type        = string
  description = "Your mode of deployment"
  default     = "SINGLE_INSTANCE"
}

variable "apply_immediately" {
  type        = bool
  description = "Specifies whether any broker modifications are applied immediately, or during the next maintenance window."
  default     = true
}

variable "auto_minor_version_upgrade" {
  type        = bool
  description = "Whether to automatically upgrade to new minor versions of brokers as Amazon MQ makes releases available."
  default     = false
}

variable "publicly_accessible" {
  type        = bool
  description = "Whether to enable connections from applications outside of the VPC that hosts the broker's subnets."
  default     = false
}

variable "tags" {
  type        = map(string)
  description = "(optional) Tags"
  default = {
    terraform = "yes"
  }
}

variable "username" {
  type        = string
  description = "(optional) description"
  default = "chaikin-dev"
}

variable "password" {
  type        = string
  sensitive   = true
  description = "(optional) description"
  default = "4rfv2wsx!qAz"
}

variable "kms_key_id" {
  type        = string
  description = "(optional) description"
  default     = null
}

variable "use_aws_owned_key" {
  type        = bool
  description = "(optional) description"
  default     = false
}

variable "maintenance_window_start_time" {
  type = object({
    day_of_week = string
    time_of_day = string
    time_zone   = string
  })
  description = "Configuration block for the maintenance window start time."
  default = {
    day_of_week = "MONDAY"
    time_of_day = "22:45"
    time_zone   = "EST/USA(GMT-5)"
  }

}

variable "logs_general" {
  type        = bool
  description = "(optional) description"
  default     = true
}